...
export async function generateMetadata({
  ...
  const page = t.devtoysAlternative as Record<string, string>;

  return generateSEO({
    title: page.metaTitle,
    description: page.metaDescription,
    path: `/${lang}/devtoys-alternative/`,
    lang: lang as Locale,
  });
}

export default async function DevToysAlternativePage({
  params,
}: {
  params: Promise<{ lang: string }>;
}) {
  const { lang } = await params;
  const t = await getDictionary(lang as Locale);
  const page = t.devtoysAlternative as {
    title: string;
    intro: string;
    whyPureKit: string;
    comparisonTitle: string;
    features: { name: string; purekit: boolean; devtoys: boolean }[];
